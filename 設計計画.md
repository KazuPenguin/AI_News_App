...
- [2026/2/20] **Frontend CI/CD の実装** (完了)
    - GitHub Actions によるテスト・Lintの自動化 (`frontend.yml`)
    - Expo EAS Update を活用した PR プレビュー環境の自動アサイン機能 (`expo-preview.yml`)
- [ ] **パーソナライズ & 多言語対応の組み込み**
    - 興味タグに基づく推薦ロジックの実装
...
- [x] **キュレーション・パイプライン評価** (2026/2/20 実装済)
    - L1→L2→L3 各段階の通過率測定（目標: L3到達率 10%以下） → `CurationStats` としてログ出力済
    - アンカー類似度閾値のチューニング（Precision/Recall トレードオフ）
    - LLM分析の分類精度・要約品質の検証
    - 1日あたりのトークン消費量の計測とコスト最適化 → `batch_logs` に `l3_input_tokens`, `l3_output_tokens`, `l3_cost_usd` の記録機能を実装済
- [ ] **RAG精度評価 (Eval)**
    - 代表的な論文に対する要約結果の検証
...
## Phase 7: 追加機能開発（Post-Release Backlog）
初期リリース後に着手する、ユーザー体験の向上とマネタイズに向けた新規開発項目です。

- [ ] **課金機能（フリー/プレミアム）の実装**
    - 決済基盤（RevenueCat / Stripe）の選定とインテグレーション
    - ユーザーのサブスクリプション状況に応じた機能制限（Paywall）の実装
- [ ] **管理用 Web アプリの構築**
    - Retool等を用いた、ユーザー管理・コンテンツ管理用ダッシュボードの開発
    - 運用・CS対応に必要な専用APIの実装